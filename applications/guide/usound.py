...
class sound:
    ...
    def msg_intro(self):
        self.clear(None)
        self.msgwin.clear()
        self.msgwin.showtext('Connect Piezo Ground wires to GND. Connect Transmitter Piezo to Digital '+\
        'output D3. Connect the Receiver Piezo output to two amplifiers in series (gain 100 x gain 10). '+\
        'Connect the amplifier output to CH0 through level shifter and '+\
        'more Inverting amplifier in series and')

        self.msgwin.showlink('View Waveform', self.show_waveform)
        
        self.msgwin.showlink('\nSave Last Trace', self.save)
        ...
        self.msgwin.showtext(' this window.\n')

        self.msgwin.showtext('Connect the Amplifier Output '+\
        'to Digital Input D3 through a 1K Series resistance and ')
        self.msgwin.showlink('Measure Time of Flight', self.ms_delay)
        self.msgwin.showtext('\n')
